	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_err("%s  st->chip_config.enable%d\n", __func__, st->chip_config.enable);
	if (result)
		return result;
	return count;
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_err("%s %d, %d\n", __func__, st->chip_config.is_asleep, st->chip_config.enable);
	if (st->chip_config.is_asleep)
		return -EPERM;
	if (st->chip_config.enable)
		return -EPERM;
	...
	if (result) {
		st->chip_config.accl_enable ^= 1;
		pr_err("%s result accl_enable[%ld]\n", __func__, st->chip_config.accl_enable);
		return result;
	}
	pr_err("%s count accl_enable[%ld]\n", __func__, st->chip_config.accl_enable);
	return count;
}
...
//printk("compass input 2bus %d, %d, %d, ns=%lld\n", x, y, z, t);        
...
//static struct regulator *gry_reg;
...
/*  	int rc=0;
	gry_reg = regulator_get(NULL,"vdd_sys_sensor_3v3");
	if(IS_ERR(gry_reg)){
		printk(KERN_ERR "%s: get gyr vaux2 regulator fail\n", __FILE__);
		rc= PTR_ERR(gry_reg);
		return rc;
	}
	rc = regulator_set_voltage(gry_reg, 2800000, 2800000);
	if(rc){
		printk(KERN_ERR "set gyr vaux2 regulator fail\n");
		return rc;
	}
	regulator_enable(gry_reg); 
	msleep(5);  */
